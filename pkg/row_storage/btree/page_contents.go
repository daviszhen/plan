...
func copyFixedHikey(
	...
	src := BTPageGetHikey(p)
	copyFixedKey(desc, dst, src)
}

func pageGetHikey(p unsafe.Pointer) Tuple {
	header := (*BTPageHeader)(p)
	util.AssertFunc(!PageIs(p, BTREE_FLAG_RIGHTMOST))
	chunkDesc := header.GetChunkDesc(int(header.chunksCount - 1))
	var result Tuple
	result.formatFlags = uint8(chunkDesc.GetHikeyFlags())
	result.data = util.PointerAdd(
		p,
		int(ShortGetLocation(chunkDesc.GetHikeyShortLocation())),
	)
	return result
}

func pageGetHikeySize(p unsafe.Pointer) LocationIndex {
	header := (*BTPageHeader)(p)
	util.AssertFunc(!PageIs(p, BTREE_FLAG_RIGHTMOST))
	chunkDesc := header.GetChunkDesc(int(header.chunksCount - 1))
	return LocationIndex(header.hikeysEnd -
