...
func TestPageSplitChunkIfNeeded(t *testing.T) {
	t.Run("basic split test", func(t *testing.T) {
		...
		header.chunksCount = 1
		header.hikeysEnd = OffsetNumber(BT_PAGE_HEADER_CHUNK_DESC_OFFSET)
		header.dataSize = BLOCK_SIZE / 2
		header.maxKeyLen = 16
		...
		chunkSize := LocationIndex(100)

		// Initialize chunk descriptor
		chunkDesc := header.GetChunkDesc(int(chunkOffset))
		chunkDesc.SetOffset(0)
		chunkDesc.SetShortLocation(LocationGetShort(uint32(BT_PAGE_HEADER_SIZE)))
		...
		// 2. Check if the hikey was set correctly
		...
		// 3. Check if the hikey flags were set
		...
		hikeyData := (*[1 << 30]byte)(util.PointerAdd(pagePtr, int(hikeyLocation)))[:8]
		for i := 0; i < 8; i++ {
			if hikeyData[i] != 0 {
				t.Errorf("Expected hikey data[%d] to be 0, got %d", i, hikeyData[i])
			}
		}
	})

	t.Run("leaf page split test", func(t *testing.T) {
		// Create a test page
		page := make([]byte, BLOCK_SIZE)
		pagePtr := unsafe.Pointer(&page[0])
	...
	t.Run("no split needed test", func(t *testing.T) {
		// Create a test page
		page := make([]byte, BLOCK_SIZE)
		pagePtr := unsafe.Pointer(&page[0])
		...
		// Create a chunk with some items
		chunkOffset := OffsetNumber(0)
		chunkItemsCount := OffsetNumber(1) // Only one item, no split needed
		chunkSize := LocationIndex(100)
		...
		// Verify the results
		// 1. Check if the chunk was not split
		if header.chunksCount != 1 {
			t.Errorf("Expected chunksCount to be 1, got %d", header.chunksCount)
		}

		// 2. Check if the hikey was not set
		newChunkDesc := header.GetChunkDesc(int(chunkOffset))
		hikeyLocation := ShortGetLocation(newChunkDesc.GetHikeyShortLocation())
		if hikeyLocation != 0 {
			t.Error("Expected hikey location to not be set")
		}

		// 3. Check if the hikey flags were not set
		hikeyFlags := newChunkDesc.GetHikeyFlags()
		if hikeyFlags != 0 {
			t.Error("Expected hikey flags to not be set")
		}
	})

	t.Run("non-leaf page split test", func(t *testing.T) {
		// Create a test page
		page := make([]byte, BLOCK_SIZE)
		pagePtr := unsafe.Pointer(&page[0])
		...
		// Initialize page header
		header := (*BTPageHeader)(pagePtr)
		header.SetFlags(0) // Non-leaf page
		header.chunksCount = 1
		header.hikeysEnd = OffsetNumber(BT_PAGE_HEADER_CHUNK_DESC_OFFSET)
		header.dataSize = BLOCK_SIZE / 2
		header.maxKeyLen = 16
	...
	t.Run("maximum chunk size test", func(t *testing.T) {
		// Create a test page
		page := make([]byte, BLOCK_SIZE)
		pagePtr := unsafe.Pointer(&page[0])
		...
		header.chunksCount = 1
		header.hikeysEnd = OffsetNumber(BT_PAGE_HEADER_CHUNK_DESC_OFFSET)
		header.dataSize = BLOCK_SIZE - 100 // Leave some space
		header.maxKeyLen = 16
		...
		// Create a chunk with maximum size
		chunkOffset := OffsetNumber(0)
		chunkItemsCount := OffsetNumber(BT_PAGE_MAX_CHUNK_ITEMS)
		chunkSize := LocationIndex(BLOCK_SIZE - 200) // Large chunk size
		...
		locator := &BTPageItemLocator{
			...
			chunkItemsCount: chunkItemsCount,
			chunkSize:       chunkSize,
			itemOffset:      chunkItemsCount / 2, // Split in the middle
			chunk:           (*BTPageChunk)(util.PointerAdd(pagePtr, int(ShortGetLocation(chunkDesc.GetShortLocation())))),
		}
	...
	t.Run("split at different points test", func(t *testing.T) {
		// Create a test page
		page := make([]byte, BLOCK_SIZE)
		pagePtr := unsafe.Pointer(&page[0])
		...
		splitPoints := []OffsetNumber{1, 5, 9}
		for _, splitPoint := range splitPoints {
			...
			// 1. Check if the chunk was split
			if header.chunksCount != 2 {
				t.Errorf("Expected chunksCount to be 2 for split point %d, got %d", splitPoint, header.chunksCount)
			}
			...
			hikeyLocation := ShortGetLocation(newChunkDesc.GetHikeyShortLocation())
			if hikeyLocation == 0 {
				t.Errorf("Expected hikey location to be set for split point %d", splitPoint)
			}

			// 3. Check if the hikey flags were set
			hikeyFlags := newChunkDesc.GetHikeyFlags()
			if hikeyFlags == 0 {
				t.Errorf("Expected hikey flags to be set for split point %d", splitPoint)
			}
			...
			for i := 0; i < 8; i++ {
				if hikeyData[i] != 0 {
					t.Errorf("Expected hikey data[%d] to be 0 for split point %d, got %d", i, splitPoint, hikeyData[i])
				}
			}
