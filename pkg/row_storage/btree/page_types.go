...
func BTPageLocatorFirst(p unsafe.Pointer, locator *BTPageItemLocator) {
	pageItemFillLocator(p, 0, locator)
}

func BTPageLocatorLast(p unsafe.Pointer, locator *BTPageItemLocator) {
	pageItemFillLocatorBackwards(p,
		BTPageItemsCount(p)-1,
		locator)
}

func BTPageLocatorTail(p unsafe.Pointer, locator *BTPageItemLocator) {
	pageItemFillLocatorBackwards(p,
		BTPageItemsCount(p),
		locator)
}

func BTPageLocatorNext(p unsafe.Pointer, locator *BTPageItemLocator) bool {
	locator.itemOffset++
	if locator.itemOffset < locator.chunkItemsCount {
		return true
	}
	return pageLocatorNextChunk(p, locator)
}

func BTPageLocatorPrev(p unsafe.Pointer, locator *BTPageItemLocator) bool {
	if locator.itemOffset > 0 {
		locator.itemOffset--
	}

	return pageLocatorPrevChunk(p, locator)
}

func BTPageLocatorIsValid(p unsafe.Pointer, locator *BTPageItemLocator) bool {
	return locator.chunk != nil &&
		locator.itemOffset < locator.chunkItemsCount
}

func BTPageLocatorSetInvalid(locator *BTPageItemLocator) {
	locator.chunk = nil
}

func BTPageLocatorGetItem(p unsafe.Pointer, locator *BTPageItemLocator) unsafe.Pointer {
	return util.PointerAdd(
		unsafe.Pointer(locator.chunk),
		int(ItemGetOffset(locator.chunk.GetItem(int(locator.itemOffset)))))
}

func BTPageGetItemFlags(p unsafe.Pointer, locator *BTPageItemLocator) uint16 {
	return ItemGetFlags(locator.chunk.GetItem(int(locator.itemOffset)))
}

func BTPageGetItemSize(p unsafe.Pointer, locator *BTPageItemLocator) LocationIndex {
...
func BTPageGetHikey(p unsafe.Pointer) Tuple {
	return pageGetHikey(p)
}

func BTPageGetHikeySize(p unsafe.Pointer) LocationIndex {
	return pageGetHikeySize(p)
}
...
const (
	BT_NON_LEAF_TUPHDR_SIZE = (uint32(unsafe.Sizeof(BTNonLeafTuphdr{})) + 7) & ^uint32(7)
	BT_LEAF_TUPHDR_SIZE     = (uint32(unsafe.Sizeof(BTLeafTuphdr{})) + 7) & ^uint32(7)
	BT_MAX_TUPLE_SIZE       = ((BLOCK_SIZE-BT_PAGE_HEADER_SIZE)/3 - LocationIndexSize - BT_LEAF_TUPHDR_SIZE) & ^uint32(7)
	BT_MAX_KEY_SIZE         = BT_MAX_TUPLE_SIZE
)
...
func TupleIsNULL(tuple Tuple) bool {
	return tuple.data == nil
}

func TupleSetNULL(tuple *Tuple) {
	tuple.data = nil
	tuple.formatFlags = 0
}
...
type FixedTuple struct {
	tuple     Tuple
	fixedData [BT_MAX_TUPLE_SIZE]byte
}
