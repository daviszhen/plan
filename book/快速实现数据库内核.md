...
围绕这一目的，编写了较完整的数据库内核系统，并完成tpch基准测试得出正确结果。
...
- [第一章 bind](#第一章-bind)
  ...
  - [解析表定义](#解析表定义)
    ...
    - [函数](#函数)
      ...
      - [cast](#cast)
  - [小结](#小结)
- [第二章 生成plan](#第二章-生成plan)
  - [简介](#简介-1)
  - [生成查询计划](#生成查询计划)
    - [生成table scan节点](#生成table-scan节点)
    - [转换子查询](#转换子查询)
      - [展开子查询](#展开子查询)
    - [生成filter节点](#生成filter节点)
    - [生成Agg节点](#生成agg节点)
    - [生成Project节点](#生成project节点)
    - [生成order节点](#生成order节点)
    - [生成limit节点](#生成limit节点)
  - [小结](#小结-1)
- [第三章 优化plan](#第三章-优化plan)
  - [简介](#简介-2)
  - [filter下推](#filter下推)
  - [join定序](#join定序)
    - [join order 优化框架](#join-order-优化框架)
    - [extractJoinRelations](#extractjoinrelations)
    - [collectRelation](#collectrelation)
    - [createEdge](#createedge)
    - [join order贪心算法](#join-order贪心算法)
    - [emitPair](#emitpair)
    - [getRelation](#getrelation)
    - [generateCrossProduct](#generatecrossproduct)
    - [rewritePlan](#rewriteplan)
  - [列裁剪](#列裁剪)
  - [生成列引用计数](#生成列引用计数)
  - [生成输出表达式](#生成输出表达式)
  - [小结](#小结-2)
- [第四章 vector](#第四章-vector)
  - [简介](#简介-3)
  - [vector](#vector)
    - [flatten](#flatten)
    - [ToUnifiedFormat](#tounifiedformat)
    - [slice](#slice)
    - [SetValute/GetValue](#setvalutegetvalue)
    - [GetSliceInPhyFormatXXX](#getsliceinphyformatxxx)
    - [GetMaskInPhyFormatXXX](#getmaskinphyformatxxx)
    - [Copy](#copy)
  - [chunk](#chunk)
  - [小结](#小结-3)
- [第五章 执行plan](#第五章-执行plan)
  - [简介](#简介-4)
  - [执行框架](#执行框架)
  - [表达式执行器](#表达式执行器)
    - [初始化](#初始化)
  ...
  - [算子](#算子)
    ...
    - [order](#order)
      ...
      - [输入数据](#输入数据-1)
        - [分配内存块](#分配内存块)
        - [转换排序键的定长部分](#转换排序键的定长部分)
        - [转换排序键的非定长部分和payload](#转换排序键的非定长部分和payload)
      - [排序](#排序)
        - [分配排序内存](#分配排序内存)
        - [排序](#排序-1)
          - [逻辑层](#逻辑层)
      ...
      - [读排序结果](#读排序结果)
    - [join](#join)
      - [非cross join](#非cross-join)
        - [初始化](#初始化-3)
        - [输入数据](#输入数据-2)
        - [执行](#执行-1)
          - [inner join](#inner-join)
          - [mark, anti mark join](#mark-anti-mark-join)
          - [semi, anti semi join](#semi-anti-semi-join)
          - [left join](#left-join)
...
- [第七章 事务](#第七章-事务)
  ...
  - [事务对象](#事务对象)
    - [事务提交操作](#事务提交操作)
      - [LocalStorage提交操作](#localstorage提交操作)
      - [UndoBuffer提交操作](#undobuffer提交操作)
    - [事务回滚操作](#事务回滚操作)
      - [LocalStorage回滚操作](#localstorage回滚操作)
      - [UndoBuffer回滚操作](#undobuffer回滚操作)
  - [walog](#walog)
  - [replay](#replay)
  - [checkpoint](#checkpoint)
    - [checkpoint执行框架](#checkpoint执行框架)
    - [持久化数据](#持久化数据)
      - [列的持久化](#列的持久化)
      - [RowGroup的持久化](#rowgroup的持久化)
      - [表的持久化](#表的持久化)
      - [Schema的持久化](#schema的持久化)
    - [数据layout](#数据layout)
    - [读取checkpoint](#读取checkpoint)
  - [小结](#小结-5)
- [第八章 插入数据](#第八章-插入数据)
  - [简介](#简介-7)
  - [状态](#状态)
  - [插入数据过程](#插入数据过程)
    ...
    - [结束](#结束)
  - [小结](#小结-6)
- [第九章 update数据](#第九章-update数据)
  - [简介](#简介-8)
  - [update过程](#update过程)
    - [确定RowGroup](#确定rowgroup)
    - [取旧值](#取旧值)
    - [构建版本信息](#构建版本信息)
      - [创建版本链表](#创建版本链表)
      - [有版本链表](#有版本链表)
  - [小结](#小结-7)
- [第十章 delete数据](#第十章-delete数据)
  - [简介](#简介-9)
  - [delete过程](#delete过程)
  - [小结](#小结-8)
- [第十一章 读数据与MVCC](#第十一章-读数据与mvcc)
  - [简介](#简介-10)
  - [状态](#状态-1)
  - [读数据过程](#读数据过程)
    - [初始化](#初始化-6)
    - [读过程](#读过程)
      ...
      - [读vector](#读vector)
      - [读已经提交过程](#读已经提交过程)
      - [checkpoint读](#checkpoint读)
  - [小结](#小结-9)
- [第十二章 catalog](#第十二章-catalog)
  - [简介](#简介-11)
  - [entry](#entry)
  - [entry集合](#entry集合)
  - [创建entry](#创建entry)
    - [取CatalogEntry](#取catalogentry)
    - [维护依赖关系](#维护依赖关系)
    - [commit操作](#commit操作)
    - [undo操作](#undo操作)
  - [小结](#小结-10)
...
表数据分成多层结构。从上到下，
第一层，由row group组成的二叉排序树。每个row group有122880行（120K）组成。
...
- 提交事务。为事务分配提交时间戳。生成redo log并落盘。有可能触发checkpoint。
- 撤销事务。回滚事务数据。
- GC事务。在合适的时间回收事务对象。事务已经提交，但是事务的数据有可能被其他事务引用，还不能立即从系统删除事务对象。
